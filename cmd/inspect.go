...
files present, settings, context sources, and Claude Code data status.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Use stored transcript path if available, otherwise compute from workspace root
			transcriptPath := sess.Metadata.TranscriptPath
			if transcriptPath == "" {
				homeDir, err := util.HomeDir()
				if err == nil {
					projectDir := claude.ProjectDir(projectClotildeRootForSession(sess))
