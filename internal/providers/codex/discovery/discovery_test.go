...
func TestScannerReturnsRolloutSessionMeta(t *testing.T) {
	codexHome := t.TempDir()
	...
	results, err := NewScanner(codexHome).Scan()
	if err != nil {
	...
	if got.Provider != session.ProviderCodex {
		t.Fatalf("Provider = %q, want %q", got.Provider, session.ProviderCodex)
	}
	...
	if got.PrimaryArtifactKind != primaryArtifactKindRollout {
		t.Fatalf("PrimaryArtifactKind = %q, want %q", got.PrimaryArtifactKind, primaryArtifactKindRollout)
	}
...
func TestScannerMarksThreadSpawnSubagents(t *testing.T) {
	codexHome := t.TempDir()
	rolloutDir := filepath.Join(codexHome, "sessions", "2026", "05", "02")
	if err := os.MkdirAll(rolloutDir, 0o755); err != nil {
		t.Fatalf("mkdir rollout dir: %v", err)
