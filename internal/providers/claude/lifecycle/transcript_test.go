...
{"type":"assistant","timestamp":"2025-01-01T10:00:20Z","message":{"model":"claude-sonnet-4-5-20250929","content":"second"}}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rawModel, ts := claudelifecycle.ExtractRawModelAndLastTime(path)
			model := claudelifecycle.FormatModelFamily(rawModel)
...
func TestExtractRawModelAndLastTime_NonExistentFile(t *testing.T) {
	model, ts := claudelifecycle.ExtractRawModelAndLastTime("/non/existent/path")
	if model != "" {
...
func TestExtractRawModelAndLastTime_EmptyPath(t *testing.T) {
	model, ts := claudelifecycle.ExtractRawModelAndLastTime("")
	if model != "" {
		t.Errorf("expected empty model, got %q", model)
	}
	if !ts.IsZero() {
...
{"type":"assistant","timestamp":"2025-01-01T10:00:20Z","message":{"model":"claude-sonnet-4-5-20250929","content":"second"}}`
	...
	model, ts := claudelifecycle.ExtractRawModelAndLastTime(path)
