...
func (c *countBackend) Count(ctx context.Context, content []compact.OutputBlock, model string) (int, error) {
	...
	if effectiveModel == "" {
		return 0, fmt.Errorf("sessionctx.count: no model resolved (layer default empty and CountOptions.Model empty)")
	}
	if c.apiKey == "" {
		return 0, fmt.Errorf("sessionctx.count: no api key configured for count_tokens")
	}
