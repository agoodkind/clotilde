...
func TestTranslateRequestAudioRejected(t *testing.T) {
    ...
    if err == nil || !errors.Is(err, tooltrans.ErrAudioUnsupported) {
...
func TestTranslateRequestToolChoiceVariants(t *testing.T) {
    ...
    tests := []struct {
        name string
        raw  string
        want *tooltrans.AnthToolChoice
    }{
        {"none", `"none"`, &tooltrans.AnthToolChoice{Type: "none"}},
        {"auto", `"auto"`, &tooltrans.AnthToolChoice{Type: "auto"}},
        {"required", `"required"`, &tooltrans.AnthToolChoice{Type: "any"}},
        {"named", `{"type":"function","function":{"name":"X"}}`, &tooltrans.AnthToolChoice{Type: "tool", Name: "X"}},
    }
