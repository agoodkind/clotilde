...
func newTranslatorForTest() *tooltrans.StreamTranslator {
    return tooltrans.NewStreamTranslator("req1", "clyde-test")
}

func TestStreamTranslatorThinkingEmitsBlockquoteContent(t *testing.T) {
    tr := newTranslatorForTest()
    var out []tooltrans.OpenAIStreamChunk
    ...
    }{Index: 0, ContentBlock: struct{ Type string `json:"type"` }{Type: "thinking"}})
    if err != nil {
        t.Fatal(err)
    }
    ...
    deltaThinking, err := json.Marshal(struct {
        Index int                `json:"index"`
        Delta *tooltrans.AnthSSEDelta `json:"delta"`
    }{Index: 0, Delta: &tooltrans.AnthSSEDelta{Type: "thinking_delta", Thinking: "step one"}})
    if err != nil {
        t.Fatal(err)
    }
...
func TestStreamTextTranslator(t *testing.T) {
    t.Parallel()
    tr := tooltrans.NewStreamTranslator("chatcmpl-s", "alias")
    var all []tooltrans.OpenAIStreamChunk
...
func TestStreamToolTranslator(t *testing.T) {
    t.Parallel()
    tr := tooltrans.NewStreamTranslator("chatcmpl-t", "alias")
    var all []tooltrans.OpenAIStreamChunk
