...
func TestTranslateRequestImageDataURI(t *testing.T) {
	...
	req := OpenAIRequest{
		...
		Messages: []OpenAIMessage{{
			Role:    "user",
			Content: json.RawMessage(`[{"type":"image_url","image_url":{"url":"data:image/png;base64,iVBOR"}}]`),
		}},
	}
	out, err := TranslateRequest(req, "", 64)
	if err != nil {
...
func TestTranslateRequestImageHTTPSURL(t *testing.T) {
	...
	req := OpenAIRequest{
		...
		Messages: []OpenAIMessage{{
			Role:    "user",
			Content: json.RawMessage(`[{"type":"image_url","image_url":{"url":"https://x/y.png"}}]`),
		}},
	}
	out, err := TranslateRequest(req, "", 64)
	if err != nil {
...
func TestTranslateRequestAudioRejected(t *testing.T) {
	...
	req := OpenAIRequest{
		...
		Messages: []OpenAIMessage{{
			Role:    "user",
			Content: json.RawMessage(`[{"type":"input_audio","input_audio":{"data":"qqq"}}]`),
		}},
	}
...
func TestTranslateRequestToolsTranslated(t *testing.T) {
	...
	req := OpenAIRequest{
		...
		Tools: []OpenAITool{
			{Type: "function", Function: OpenAIToolFunctionSchema{Name: "a", Parameters: json.RawMessage(`{"type":"object"}`)}},
			{Type: "function", Function: OpenAIToolFunctionSchema{Name: "b", Description: "d", Parameters: json.RawMessage(`{"a":1}`)}},
		},
	}
	out, err := TranslateRequest(req, "", 64)
	if err != nil {
