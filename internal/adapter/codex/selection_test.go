...
func TestResolveTransportSelectionKeepsDirectOnHealthyPath(t *testing.T) {
	...
	path, res, managed, err := resolveTransportSelection(
		...
		nil,
		false,
		func() (RunResult, bool, error) { t.Fatal("fallback should not run"); return RunResult{}, false, nil },
	)
	if err != nil {
		t.Fatalf("err=%v", err)
	}
...
func TestResolveTransportSelectionFallsBackToApp(t *testing.T) {
	...
	path, res, managed, err := resolveTransportSelection(
		...
		nil,
		true,
		func() (RunResult, bool, error) { return RunResult{FinishReason: "stop"}, true, nil },
	)
	if err != nil {
		t.Fatalf("err=%v", err)
	}
	...
	if res.FinishReason != "stop" {
		t.Fatalf("res=%v want stop result", res)
	}
	if !managed {
...
func TestResolveTransportSelectionReturnsFallbackError(t *testing.T) {
	...
	path, _, _, err := resolveTransportSelection(
		...
		nil,
		false,
		func() (RunResult, bool, error) { return RunResult{}, false, fallbackErr },
	)
