...
- Live-validated the CLYDE-134 basic Claude CLI ingress shape with `ANTHROPIC_BASE_URL` pointed at a local Anthropic-shaped capture server. `claude -p "reply with ok" --model claude-sonnet-4-5-20250929` completed and called only `POST /v1/messages?beta=true` with `stream=true`; it did not call `/v1/models` or `/v1/messages/count_tokens` on that path. Added route coverage so Clyde preserves native SSE framing when Claude appends the `beta=true` query.
- Reconciled CLYDE-150 against the live tree: `clyde mitm diff` now auto-detects SnapshotV2 references, `wire-snapshot-check` now uses `reference-v2.toml` plus `--v2` when present, and the plan doc now marks the missing committed Claude Code v2 reference, live zero-divergence run, and drift log evidence as still open. Local checks found only `~/.local/state/clyde/mitm/always-on/capture.jsonl` with 6 lines, no committed `research/claude-code/snapshots/latest/reference-v2.toml`, and no local `~/.local/state/clyde/mitm-drift/` outcome file.
- Reconciled the CLYDE-149 finish-reason cleanup against the live tree: `internal/adapter/stream.go` is already gone, root fallback/bridge paths remain deleted, provider-specific terminal-state mapping is confined to provider-owned Anthropic/Codex callers of `internal/adapter/finishreason/`, and the root runtime fallback-only `cli_model` log branch plus stale fallback comments were removed.
- Closed the CLYDE-146 validation loop against existing real OAuth-bucket Anthropic 429 evidence in `~/.local/state/clyde/anthropic.jsonl`. Two `anthropic.ratelimit` events for `claude-opus-4-7` on 2026-04-29 at 21:30 PDT returned `status=429`, `retry_after` around 91k seconds, `anthropic-ratelimit-unified-status="rejected"`, `anthropic-ratelimit-unified-7d-status="rejected"`, `anthropic-ratelimit-unified-overage-status="rejected"`, `anthropic-ratelimit-unified-7d-surpassed-threshold="1.0"`, and native Anthropic body type `rate_limit_error`. Request ids and `body_b64` are intentionally omitted here.
- Redacted representative log excerpt:
  `{"time":"2026-04-29T21:30:53.812508-07:00","level":"WARN","msg":"anthropic.ratelimit","model":"claude-opus-4-7","status":429,"request_id":"[redacted]","anthropic-ratelimit-unified-status":"rejected","anthropic-ratelimit-unified-7d-status":"rejected","anthropic-ratelimit-unified-overage-status":"rejected","anthropic-ratelimit-unified-7d-surpassed-threshold":"1.0","retry_after":"91746","body":{"type":"error","error":{"type":"rate_limit_error","message":"This request would exceed your account's rate limit. Please try again later."},"request_id":"[redacted]"}}`
- Confirmed the live code path maps those responses through `ResponseClassRetryableError`: `internal/adapter/anthropic/client.go` calls `Classify(resp, nil)` for HTTP 429 after logging headers and body, `internal/adapter/anthropic/classify.go` marks 429 retryable while preserving warning flags from unified headers, and `internal/adapter/anthropic/backend/response_runtime.go` emits OpenAI-native error envelopes with `type="rate_limit_error"` and `code="rate_limit_exceeded"` for typed 429 upstream errors.
- Added focused streaming lock-in coverage in `internal/adapter/anthropic/backend/response_runtime_test.go` so a typed Anthropic 429 emits a native SSE error envelope and does not fall back to assistant-shaped actionable prose.
